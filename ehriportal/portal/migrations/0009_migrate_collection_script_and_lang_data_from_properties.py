...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for c in orm.Collection.objects.all():
            for prop in ["scripts", "languages", "scripts_of_description",
                    "languages_of_description"]:
                pname = prop.replace("languages", "language").replace(
                        "scripts", "script")
                props = [p.value for p in orm.Property.objects.filter(
                        resource=c.id, name=pname).distinct()]
                setattr(c, prop, props)
            c.save()


    def backwards(self, orm):
        "Write your backwards methods here."
        for model in [orm.Collection]:
            for c in model.objects.all():
                for prop in ["scripts", "languages", "scripts_of_description",
                        "languages_of_description"]:
                    pname = prop.replace("languages", "language").replace(
                            "scripts", "script")
                    for val in getattr(c, prop):
                        prop, created = orm.Property.objects.get_or_create(resource_id=c.id,
