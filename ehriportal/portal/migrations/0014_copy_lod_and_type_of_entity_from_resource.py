...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        db.execute("""UPDATE portal_repository t, portal_resource s SET t.tlod = s.lod
                WHERE t.resource_ptr_id = s.id""")
        db.execute("""UPDATE portal_collection t, portal_resource s SET t.tlod = s.lod
                WHERE t.resource_ptr_id = s.id""")
        db.execute("""UPDATE portal_repository t, portal_resource s SET t.ttype_of_entity = s.type_of_entity
                WHERE t.resource_ptr_id = s.id""")
        db.execute("""UPDATE portal_collection t, portal_resource s SET t.ttype_of_entity = s.type_of_entity
                WHERE t.resource_ptr_id = s.id""")


    def backwards(self, orm):
        "Write your backwards methods here."
        db.execute("""UPDATE portal_resource t, portal_repository s SET t.lod = s.tlod
                WHERE s.resource_ptr_id = t.id""")
        db.execute("""UPDATE portal_resource t, portal_collection s SET t.lod = s.tlod
                WHERE s.resource_ptr_id = t.id""")
        db.execute("""UPDATE portal_resource t, portal_repository s SET t.type_of_entity = s.ttype_of_entity
                WHERE s.resource_ptr_id = t.id""")
        db.execute("""UPDATE portal_resource t, portal_collection s SET t.type_of_entity = s.ttype_of_entity
                WHERE s.resource_ptr_id = t.resource_ptr_id""")
