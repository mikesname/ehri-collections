...
# Schema and data migration to transfer charfields to positiveinteger fields
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        for table, lookups in LOOKUPS.items():
            for column, vals in lookups.items():
                opts, trans = vals
                db.add_column(table, column + "_temp", 
                        self.gf('django.db.models.fields.PositiveIntegerField')(**opts))
                db.start_transaction()
                for v1, v2 in trans.items():
                    # yeah, we don't use parameterised queries... I know
                    db.execute("UPDATE %s SET %s_temp=%d WHERE %s='%s'" % (table, column, v2, column, v1))
                db.commit_transaction()
                db.delete_column(table, column)
                db.rename_column(table, column + "_temp", column)
    ...
    def backwards(self, orm):
        ...
        for table, lookups in LOOKUPS.items():
            for column, vals in lookups.items():
                opts, trans = vals
                opts.update(max_length=255)
                db.add_column(table, column + "_temp", 
                        self.gf('django.db.models.fields.CharField')(**opts))
                db.start_transaction()
                for v1, v2 in trans.items():
                    # yeah, we don't use parameterised queries... I know
                    db.execute("UPDATE %s SET %s_temp=%s WHERE %s=%d" % (table, column, v1, column, v2))
                db.commit_transaction()
                db.delete_column(table, column)
                db.rename_column(table, column + "_temp", column)
